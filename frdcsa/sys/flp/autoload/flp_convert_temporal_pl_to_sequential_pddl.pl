...
flp_convert_temporal_pl_to_sequential_pddl(Arguments) :-
	view([flp_convert_temporal_pl_to_sequential_pddl]),
...
outputSequential(domain(Domain),DomainResults) :-
	...
	fixActionsSequential(TmpActions,Actions),
...
fixPreconditionsSequential(InputPreconditions,OutputPreconditions) :-
...
fixEffectsSequential(InputEffects,OutputEffects) :-
	findall(Effect,
	 	(   
...
fixActionsSequential(TmpActions,Actions) :-
	findall(Action,
		(   
		    ...
		    fixPreconditionsSequential(TmpPreconditions,Preconditions),
		    fixEffectsSequential(TmpEffects,Effects),
		    Action = ':action'(ActionName,':parameters',Parameters,':precondition',Preconditions,':effect',Effects)
		),
		Actions),
	view([actions,Actions]).

fixObjectsSequential(TmpObjects,Objects) :-
...
fixInitSequential(TmpInits,Init) :-
	findall(TmpInit,
		(   
		    member(Tmp,TmpInits),
		    (	atomic(Tmp) ->
			...
			(   Tmp =.. [TmpPred|TmpRest],
				...
				(   fixFormulaSequential(TmpFluent,Fluent),
				    Rest = [Fluent,Value]) ;
				Rest = TmpRest),
			    TmpInit =.. [Pred|Rest]))
		),
...
fixFormulaSequential(Formula,CorrectedFormula) :-
	(   atomic(Formula) ->
...
outputSequential(problem(Problem),ProblemResults) :-
	...
	fixObjectsSequential(TmpObjects,Tmp2Objects),
